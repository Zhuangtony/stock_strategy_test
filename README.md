...
- 若瀏覽器顯示「無法連線至 Yahoo Finance」或「Failed to fetch」，通常與 Yahoo Finance 的跨來源限制或暫時性網路問題有關。前端已經自動在 `query1.finance.yahoo.com` 與 `query2.finance.yahoo.com` 之間切換來源，若兩者皆無法連線，建議：
  - 確認網頁是透過 HTTPS 服務（例如 GitHub Pages 或 `python -m http.server`）載入，而非直接以 `file://` 開啟。
  - 稍等數分鐘後再執行模擬，或換用不同的網路環境。
